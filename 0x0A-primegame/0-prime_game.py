...
def isWinner(x, nums):
    ...
    if x != len(nums):
        return None
    # Initializes the scores and array of possible prime numbers
    ben = 0
    maria = 0
    # Creates a list 'a' of length sorted(nums)[-1] + 1 with all elements
    # initialized to 1
    a = [1 for x in range(sorted(nums)[-1] + 1)]
    # The first two elements of the list, a[0] and a[1], are set to 0
    # because 0 and 1 are not prime numbers
    a[0], a[1] = 0, 0
    # Using Sieve of Eratosthenes algorithm to generate array of prime numbers
